...
import { minimalFlow, simpleId, asyncTimeout, end, start, next, prev, setId } from './utils.js'
...
describe('Tinyflow - integration', function () {
  ...
  describe('most minimal workflow', () => {
    it('runs with the most minimal definitions', async () => {
      ...
      expect(Tinyflow.get(instanceId)).to.equal(wf)
      ...
      expect(Tinyflow.get(instanceId)).to.equal(undefined)
    })
  ...
  describe('Storage, persistence and restore', () => {
    it('allows to attach a storage', async () => {
      const workflows = new Map()
      const steps = new Map()

      Tinyflow.use('storage', (config, { workflow, step }) => {
        if (workflow) {
          const update = () => workflows.set(workflow.id, JSON.stringify(workflow))
          workflow.on('started', update)
          workflow.on('step', update)
          workflow.on('end', update)
        }

        if (step) {
          const update = () => steps.set(step.id, JSON.stringify(step))
          step.on('started', update)
          step.on('update', update)
          step.on('end', update)
        }
      })
      ...
      expect(workflows.has(wf.id)).to.equal(false)
      await start(wf)
      await next(wf)
      await end(wf)
      expect(workflows.get(wf.id)).to.equal(JSON.stringify(wf))
    })
    it('allows to restore from Storage')
  })
})